...
function toTrackingItem(update: Regulation): LegalTrackingItem {
  if (!update.isActive) {
    return {
      ...
      description:
        update.description ??
        "Quy định này không còn hiệu lực. Hãy rà soát các hồ sơ liên quan.",
  ...
  if (update.effectiveDate && new Date(update.effectiveDate) > new Date()) {
    return {
      ...
      title: update.title,
      description:
        update.description ?? `Có hiệu lực từ ${formatDate(update.effectiveDate)}.`,
...
export function LegalTrackingWidget({
  ...
  return (
    <section
      className={`bg-surface p-6 rounded-2xl border border-outline-variant shadow-xs space-y-6 ${className}`}
    ...
    >
      ...
      {isLoading && (
        <div className="space-y-4" aria-label="Đang tải cập nhật pháp lý">
          {[1, 2, 3].map((item) => (
            <div key={item} className="h-20 animate-pulse rounded-lg bg-surface-container-low" />
          ))}
        </div>
      )}
      ...
      {!isLoading && !error && items.length > 0 && (
        <div className="relative pl-6 space-y-7 before:absolute before:left-2 before:top-2.5 before:bottom-2.5 before:w-px before:bg-outline-variant">
          {items.map((item) => {
            ...
            return (
              <div key={item.id} className="relative group">
                <div
                  className={`absolute -left-[23px] top-1.5 w-3.5 h-3.5 rounded-full ${dotBg} ring-4 ring-surface shadow-xs group-hover:scale-125 transition-transform duration-200`}
                />
                <div className="space-y-1.5">
                  ...
                  {item.actionText && item.href && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="pt-1 text-sm font-semibold text-primary hover:underline flex items-center gap-1 transition-all"
                    >
                      {item.actionText}
                    </a>
                  )}
